// Generate example phrases for the HSK 2 grammar constructions defined in
// data/patterns/_hsk2_constructions.json, across cumulative vocabulary scopes.
//
// Vocab scope determines which old-syllabus HSK lists are merged into the
// allowed vocabulary:
//   --vocab-scope 2  →  HSK 1+2  →  hsk2_patterns.json
//   --vocab-scope 3  →  HSK 1+2+3  →  hsk2_3_patterns.json
//   --vocab-scope 4  →  HSK 1+2+3+4  →  hsk2_3_4_patterns.json
//   --vocab-scope 5  →  HSK 1+2+3+4+5  →  hsk2_3_4_5_patterns.json
//   --vocab-scope 6  →  HSK 1..6  →  hsk2_3_4_5_6_patterns.json
//
...
// structure. This is slower than asking for 30 phrases at once but the per-
// phrase context window prevents the duplication and homogeneity batched
// generation tends to produce.
//
// Inheritance — when --vocab-scope is N > 2, the generator boots its cache
// from the previous scope's cache (V{N-1}). Phrases produced for the smaller
// scope are valid for the larger one too (the char set is a superset), so
// they carry forward and we only generate the delta. This is what makes the
// cumulative file family practical.
//
// Resumable: progress is written into scripts/hsk2-phrase-cache-v{N}.json
// after every successful phrase. Re-running picks up wherever it stopped.
//
// Usage:
//   node scripts/generate-hsk2-patterns.mjs --vocab-scope 3 --target 50
//   node scripts/generate-hsk2-patterns.mjs --only 1,2,3 --vocab-scope 4
//   node scripts/generate-hsk2-patterns.mjs --emit --vocab-scope 5
...
const VOCAB_SCOPE = Number(val("vocab-scope", "2"));
if (![2, 3, 4, 5, 6].includes(VOCAB_SCOPE)) {
  throw new Error(`--vocab-scope must be 2..6, got ${VOCAB_SCOPE}`);
}
// Default target scales with scope (more vocab → more diverse phrases).
const TARGET_BY_SCOPE = { 2: 25, 3: 50, 4: 80, 5: 120, 6: 170 };
...
function scopeSuffix(scope) {
  // 2 → "" (legacy file: hsk2_patterns.json)
  // 3 → "_3"
  // 4 → "_3_4"
  // ...
  if (scope === 2) return "";
  const parts = [];
  for (let i = 3; i <= scope; i++) parts.push(String(i));
  return "_" + parts.join("_");
}
function patternsFileFor(scope) {
  return `hsk2${scopeSuffix(scope)}_patterns.json`;
}
function cacheFileFor(scope) {
  return `hsk2-phrase-cache-v${scope}.json`;
}
...
const constructionsFile = JSON.parse(
  readFileSync(join(patternsDir, "_hsk2_constructions.json"), "utf-8"),
);
...
function loadCache() {
  ...
  // Inherit from V{scope-1} on first run for any scope > 2 — phrases valid
...
6. Do not repeat or paraphrase any "already generated" sentence. Vary subjects (我 / 你 / 他 / 她 / 我们 / 你们 / 他们 / proper noun), verbs, and objects across sentences.${VOCAB_SCOPE > 2 ? "\n7. Reach for vocabulary the user hasn't seen yet. Earlier phrases used a narrower HSK 1+2 word pool — for new phrases, prefer words from the wider HSK 1–" + VOCAB_SCOPE + " range so each example introduces something fresh, while still respecting the whitelist." : ""}
${VOCAB_SCOPE > 2 ? 8 : 7}. Output STRICT JSON: {"zh": "…", "py": "…", "ru": "…"}. No commentary.`;
...
async function fillConstruction(construction, cache) {
  ...
  // Seed the cache with the curated seeds on first ever fill (V12). For
  // higher scopes we already inherited V{N-1}, which itself had the seeds,
  // so we skip seeding here.
  if (VOCAB_SCOPE === 2 && phrases.length === 0) {
...
function emit(cache) {
  ...
  const out = {
    level: scopeLabel(VOCAB_SCOPE).replace("HSK ", "HSK2 / V"),
    title: `HSK 2 Паттерны — грамматика HSK 2, лексика ${scopeLabel(VOCAB_SCOPE)}`,
    method: "Метод паттернов Ягодкина: изоляция + интенсивность + обратная связь",
